...
spark = SparkSession.builder     .appName("ProcessVetTwitteText")     .master("spark://node01:10077")     .enableHiveSupport()    .config("spark.driver.memory", "2g")     .config("spark.executor.memory", "2g")     .config("spark.cores.max", "3")     .config("spark.sql.shuffle.partitions", "12")     .config("spark.sql.autoBroadcastJoinThreshold", "-1")     .getOrCreate()
...
# do word count
item_title_word_count = cut_items.flatMap(title_to_count)                    .reduceByKey(lambda a,b:a+b)                    .map(split_key_set_date)
print(item_title_word_count.count())
item_content_word_count = cut_items.flatMap(content_to_count)                    .reduceByKey(lambda a,b:a+b)                    .map(split_key_set_date)
